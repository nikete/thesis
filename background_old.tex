...
%Los deterministas niegan que haya en el mundo un solo hecho posible, id est un hecho que pudo acontecer; una moneda simboliza nuestro libre albedrÃ­o. (€œEl Zahir€)
...
%With regards to maximizing the future social welfare in a dynamic setting, there are elegant extensions of the efficient (VCG) mechanism that are applicable to quite general dynamic settings by [Bergemann and V¨alim¨aki 2010; Parkes and Singh 2003; Athey and Segal 2007].
...
% % %will pay that trader z0 p(z Ìƒ)dz Ìƒ for the (z1 ˆ’z0) units. The current price of the security will move from p(z0) to p(z1). If the true value of the security then
% %turns out to be equal to x, then the trader€™s payoff from this transaction will be equal to ô°‹ z1 p(z Ìƒ) dz Ìƒ ˆ’ x(z1 ˆ’ z0) = ô°‹ z1 (p(z Ìƒ) ˆ’ x) dz Ìƒô°· Thus, it is strictly optimal
% %z0 z0
...
% %EXAMPLE1: Therearetwoagents,1and2,andfourstatesoftheworld,Î©=
% %{Aô°¸Bô°¸Cô°¸D}. The prior is P(Ï‰) = 1 for every Ï‰ ˆˆ Î©. The security is X(A) = 4
% %X(D) = 1 and X(B) = X(C) = ˆ’1. Partitions are Î 1 = {{Aô°¸B}ô°¸{Cô°¸D}} and Î 2 ={{Aô°¸C}ô°¸{Bô°¸D}}.
...
% these problems, given an optimal (or β-approximation) algorithm for an algorithmic
% pricing problem, we can convert it into a (1 + )-approximation (or β(1 + )-
...
% these settings present several challenges: in particular, the “loss function” is discontinuous,
...
% Ilan Kremer puts it this way: “A key question in information economics and finance
...
% prices that would occur if all information were public?”



% Pesendorfer and Swinkels, (“The Loser’s Curse and Information Aggregation in Common
% Value Auctions,” Econometrica 1997, recently added to the syllabus) explain
% Milgrom’s result this way: when k is fixed but n grows unboundedly large, the winner’s
...
% individual high signals are extremely powerful, “high” bids are never optimal; but
...
% the sequence satisfies double largeness if both kr and nr − kr go to infinity as r → ∞.
% • Double largeness turns out to be both necessary and sufficient for information aggregation:
% Theorem 2. The sequence of auctions {Ar} satisfies full information aggregation
% (that is, pr −v converges to 0 in probability) if and only if it satisfies double-largeness.


% Ilan Kremer (“Information Aggregation in Common Value Auctions,” Econometrica
% 2002) greatly simplifies the proofs of many of the existing results, and recasts things
% in terms of some new definitions.
% • Kremer also gives an example where signals get less informative as the number of
% bidders grows and information fails to aggregate. He does it in a bit of an “edge
% case,” but it’s still an interesting example.
% • Suppose that in auction r, there are r sellers and 2r buyers, that is, kr = r and
...
% “multi-armed bandit” describes a hypothetical experiment in which a player faces several slot
% machines (“one-armed bandits”) with potentially different expected payouts, a strategic bandit involves
...
% KRC), it follows a simple Poisson process, with positive lump-sums (“breakthroughs”) occurring
