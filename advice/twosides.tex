...
\begin{enumerate}
\item A new subject $t$ arrives and each $i$ of $K$ experts receives a signal $s_{t,i}$ for that subject. The mechanism  randomly allocates an exploration transfer of $l$ to one of the actions available to the subject. We denote this action by $l_t$.
...
Even when signal structures satisfy the single crossing property running the direct mechanism of chapter 5 repeatedly, once for each subject, results in choosing the arm with the maximum posterior expected reward at each step $t$ and using the payment rule:
...
Under full disclosure, a repeated direct reward sharing  mechanism (DRSM) in Example~\ref{eg:2regimes}, there is a NE of the repeated single subject mechanism which results in the greedy policy.
